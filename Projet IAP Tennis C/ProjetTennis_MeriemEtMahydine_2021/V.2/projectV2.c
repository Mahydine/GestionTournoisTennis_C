...
void Affichage_matchs_tournoi (TournoisWTA *t){ //printf("tournoi inconnu"); //idxPerdante idxGagnante
    ...
    printf("%s %s\n", t->dataTournois[w].nomTournoi, t->dataTournois[w].dateTournoi);
    for(int i = 6; i >= 0; --i){
...
void Afficher_matchs_joueuse(TournoisWTA *t){ // parcourir toutes les joueuses et afficher les matchs auquel correspondent la joueuse qu'on veut
    char nomtournoi[lgMot+1];
    char date [4];
    char nomJoueuseVoulue[lgMot+1];
    scanf("%s", nomtournoi);
    scanf("%s", date);
    scanf("%s", nomJoueuseVoulue);
    unsigned int w;
    unsigned int k;
    
    for (w = 0 ; w < t->idxT && strcmp(t->dataTournois[w].nomTournoi, nomtournoi) == 0 && strcmp(t->dataTournois[w].dateTournoi, date) == 0 ; ++w);
    for (k = 0; k < t->idxJ && strcmp(t->dataJoueuse[k].nom, nomJoueuseVoulue) != 0 ; k++);
    // maintenant qu'on a l'indice, il faut afficher tous les matchs où l'indaice 120 est présent que ce soit en gagnante ou en perdante
    for (int i=0 ; i<=nbMatchsTournoi; ++i){
        // parcourir tous les matchs à la case i et de regarder si idxg ou idxp correspond à k
       
        if(t->dataTournois[w].dataMatch[i].idxGagnante == k || t->dataTournois[w].dataMatch[i].idxPerdante == k ){ // on veut fouiller dans cette liste et trouver chaque fois que y'a 120
            printf("%s %s\n", t->dataJoueuse[t->dataTournois[w].dataMatch[i].idxGagnante].nom, t->dataJoueuse[t->dataTournois[w].dataMatch[i].idxPerdante].nom);
        }
    }
}
...
int main(){
    ...
    while (1){  // Cette boucle tourne à l'infini pour que l'on puisse rentrer le nombre de commande que l'on veut
        scanf("%s", Commande);
            ...
            }else if(strcmp(Commande, "afficher_matchs_joueuse") == 0) {
                Afficher_matchs_joueuse(&t);
